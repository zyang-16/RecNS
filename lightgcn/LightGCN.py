...
from utility.walks import Walk
...
class LightGCN(object):
    def __init__(self, data_config, pretrain_data):
        ...
        self.norm_adj_view = data_config['norm_adj_view']
        self.n_nonzero_elems = self.norm_adj.count_nonzero()
        self.n_nonzero_elems_view = self.norm_adj_view.count_nonzero()
        self.lr = args.lr
        ...
        *********************************************************
        ...
        self.neg_items_exposed = tf.placeholder(tf.int32, shape=(None,))
        ...
        *********************************************************
        ...
        if self.alg_type in ['lightgcn']:
            print("entering lightgcn......")
            self.ua_embeddings, self.ia_embeddings = self._create_lightgcn_embed() 
            self.ua_embeddings_view, self.ia_embeddings_view = self._create_lightgcn_embed_view() 
        ...
        *********************************************************
        ...
        self.neg_i_g_embeddings_pos = tf.nn.embedding_lookup(self.ia_embeddings, self.neg_items)
        self.neg_i_g_embeddings_exposed = tf.nn.embedding_lookup(self.ia_embeddings, self.neg_items_exposed)

        # merge embedding
        self.ids = tf.constant(np.arange(0, args.batch_size*args.K_negs, args.K_negs))
        self.neg_i_g_embeddings_pos_one = tf.nn.embedding_lookup(self.neg_i_g_embeddings_pos, self.ids)
        self.neg_i_g_embeddings_merge = (1-1.0/args.K_negs) * self.neg_i_g_embeddings_pos_one + (1.0/args.K_negs) * self.neg_i_g_embeddings_exposed
        self.neg_outputs_exposed_copy = tf.reshape(tf.tile(self.neg_i_g_embeddings_merge, [1, args.K_negs]), (-1, tf.shape(self.neg_i_g_embeddings_exposed)[-1]))
        self.neg_i_g_embeddings = (1.0-1.0/args.K_negs) * self.neg_i_g_embeddings_pos + (1.0/args.K_negs) * self.neg_outputs_exposed_copy
        ...
        *********************************************************
        ...
        self.alpha = tf.random_uniform((args.batch_size, args.K_Candidates), minval=0, maxval=1.0)
        self.all_rating_user = tf.squeeze(tf.matmul(tf.reshape(self.neg_i_g_embeddings_pos, (self.batch_size, -1, tf.shape(self.neg_i_g_embeddings_pos)[-1])), tf.expand_dims(self.u_g_embeddings, -1)))
        self.all_rating_item = tf.squeeze(tf.matmul(tf.reshape(self.neg_i_g_embeddings_pos, (self.batch_size, -1, tf.shape(self.neg_i_g_embeddings_pos)[-1])), tf.expand_dims(self.pos_i_g_embeddings, -1)))
        self.all_rating = tf.sigmoid(self.alpha * self.all_rating_user + (1 - self.alpha) * self.all_rating_item)
        self.negs_id = tf.multinomial(self.all_rating, args.K_negs)  

        # exposure-augmented sampling
        self.all_rating_user_exposed = tf.squeeze(tf.matmul(tf.reshape(self.neg_i_g_embeddings_exposed, (self.batch_size, -1, tf.shape(self.neg_i_g_embeddings_exposed)[-1])), tf.expand_dims(self.u_g_embeddings, -1)))
        self.prob_beta = tf.squeeze(tf.sigmoid(self.all_rating_user_exposed * self.beta))
        self.negs_id_exposed = tf.argmax(self.prob_beta, 1)


        """
        *********************************************************
        Generate Predictions & Optimize via the augmented hinge loss.
        """
        self.mf_loss, self.emb_loss, self.reg_loss = self.create_hinge_loss(self.u_g_embeddings,
    ...
    def _create_lightgcn_embed_view(self):
        if self.node_dropout_flag:
            A_fold_hat = self._split_A_hat_node_dropout(self.norm_adj_view)
        else:
            A_fold_hat = self._split_A_hat(self.norm_adj_view)
    ...
    def create_hinge_loss(self, users, pos_items, neg_items):
        self.u_g_embeddings_copy = tf.reshape(tf.tile(users, [1, args.K_negs]), (-1, tf.shape(users)[-1]))
        self.pos_i_g_embeddings_copy = tf.reshape(tf.tile(pos_items,[1,args.K_negs]), (-1, tf.shape(pos_items)[-1]))
        pos_scores = tf.sigmoid(tf.reduce_sum(tf.multiply(self.u_g_embeddings_copy, self.pos_i_g_embeddings_copy), axis=1))
        neg_scores = tf.sigmoid(tf.reduce_sum(tf.multiply(self.u_g_embeddings_copy, neg_items), axis=1))
        ...
        hinge_loss = tf.reduce_mean(tf.nn.relu(tf.subtract(neg_scores, pos_scores - 0.1)))

        emb_loss = self.decay * regularizer

        reg_loss = tf.constant(0.0, tf.float32, [1])

        return hinge_loss, emb_loss, reg_loss
...
class sample_thread(threading.Thread):
    def __init__(self, negative_sampling_strategy, sess, model, node_dropout_feed, mess_dropout_feed, intermediates):
        threading.Thread.__init__(self)
        self.sess = sess
        self.model = model
        self.node_dropout_feed = node_dropout_feed
        self.mess_dropout_feed = mess_dropout_feed
        self.negative_sampling_strategy = negative_sampling_strategy
        self.intermediates = intermediates
        
    def run(self):
        with tf.device(cpus[0]):
            self.data = data_generator.sample(self.negative_sampling_strategy, self.sess, self.model, self.node_dropout_feed, self.mess_dropout_feed, self.intermediates)
...
class train_thread(threading.Thread):
    ...
    def run(self):
        ...
        users, pos_items, neg_items, neg_items_exposed = self.sample.data
        self.data = sess.run([self.model.opt, self.model.loss, self.model.mf_loss, self.model.emb_loss, self.model.reg_loss],
                                feed_dict={model.users: users, model.pos_items: pos_items,
                                            ...
                                            model.neg_items_exposed: neg_items_exposed})
...
if __name__ == '__main__':
    ...
    print("K_Candidates", args.K_Candidates)
    print("K_negs", args.K_negs)
    ...
    *********************************************************
    ...
    plain_adj_view, norm_adj_view, mean_adj_view,pre_adj_view = data_generator.get_adj_mat_view()
    if args.adj_type == 'plain':
        config['norm_adj'] = plain_adj
        config['norm_adj_view'] = plain_adj_view
        print('use the plain adjacency matrix')
    elif args.adj_type == 'norm':
        config['norm_adj'] = norm_adj
        config['norm_adj_view'] = norm_adj_view
        print('use the normalized adjacency matrix')
    elif args.adj_type == 'gcmc':
        config['norm_adj'] = mean_adj
        config['norm_adj_view'] = mean_adj_view
        print('use the gcmc adjacency matrix')
    elif args.adj_type=='pre':
        config['norm_adj']=pre_adj
        config['norm_adj_view']=pre_adj_view
        print('use the pre adjcency matrix')
    else:
        config['norm_adj'] = mean_adj + sp.eye(mean_adj.shape[0])
        config['norm_adj_view'] = mean_adj_view + sp.eye(mean_adj_view.shape[0])
    ...
    if args.pretrain == 1:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            if args.report != 1:
                ...
                pretrain_ret = 'pretrained model recall=[%s], ndcg=[%s], '\
                               'hr=[%s]' % \
    ...
    *********************************************************
    ...
    if args.report == 1:
        ...
        for i, users_to_test in enumerate(users_to_test_list):
            ...
            final_perf = "recall=[%s], ndcg=[%s], hr=[%s]" % \
                         (', '.join(['%.5f' % r for r in ret['recall']]),
                          ', '.join(['%.5f' % r for r in ret['ndcg']]),
                          ', '.join(['%.5f' % r for r in ret['hits']]))
    ...
    *********************************************************
    ...
    # generate intermedidate items for negative sampling
    t_load = time()
    Walk(data_generator.train_items, data_generator.n_users, data_generator.n_items, args.khops)
    intermediates = load_candidates('candidates_zhihu_intermediates.txt')
    print("t_loading....", time() - t_load)


    loss_loger, rec_loger, ndcg_loger, hit_loger = [], [], [], []
    stopping_step = 0
    should_stop = False
    
    mess_dropout_feed = eval(args.mess_dropout)
    node_dropout_feed = eval(args.node_dropout)
    negative_sampling_strategy = args.strategy
    avg_reward = 0
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        *********************************************************
        ...
        '''
        sample_last = sample_thread(negative_sampling_strategy, sess, model, node_dropout_feed, mess_dropout_feed, intermediates)
        sample_last.start()
        sample_last.join()
        for idx in range(n_batch):
            epoch_reward = 0
            train_cur = train_thread(model, sess, sample_last)
            sample_next = sample_thread(negative_sampling_strategy, sess, model, node_dropout_feed, mess_dropout_feed, intermediates)
            ...
            users, pos_items, neg_items, neg_items_exposed = sample_last.data
            ...
            avg_reward = epoch_reward / args.batch_size
        ...
        perf_str = 'Epoch %d: train==[%.5f=%.5f + %.5f + %.5f], recall=[%s], ndcg=[%s], hr=[%s]' % \
    ...
    final_perf = "Best Iter=[%d]@[%.1f]\trecall=[%s], ndcg=[%s], hr=[%s]" % \
