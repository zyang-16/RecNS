...
def test(sess, model, users_to_test, drop_flag=False, train_set_flag=0):
    ...
    result = {'recall': np.zeros(len(model.Ks)), 'ndcg': np.zeros(len(model.Ks)), 'hits': np.zeros(len(model.Ks))}
    ...
    item_embeds = sess.run(model.pos_i_g_embeddings, {model.pos_items:item_batch, model.node_dropout: [0.]*len(eval(args.layer_size)),
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        user_embeds = sess.run(model.u_g_embeddings, {model.users:user_batch, model.node_dropout: [0.]*len(eval(args.layer_size)),
                               model.mess_dropout: [0.] * len(eval(args.layer_size))})

        D, I = index.search(user_embeds, 500)
        recommended_item = I
        rate_batch = np.array(recommended_item)
        ...
        batch_result, hr_result = eval_score_matrix_foldout(rate_batch, test_items, max_top)#(B,k*metric_num), max_top= 20
        count += len(batch_result)
        all_result.append(batch_result)
        hrs.extend(hr_result)
    ...
    result['hits'] += np.mean(np.array(hrs))
    return result
